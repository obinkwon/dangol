		...
		<div class="w90Form" style="margin:auto;text-align:center;">
			...
			<div class="paging">
				<c:if test="${viewInfo.startPage ne 1}">
					<a href="#none" class="first" onclick="themeSort('1','${viewInfo.type}','${viewInfo.anum}')">처음</a>
					<a href="#none" class="prev" onclick="themeSort('${viewInfo.startPage-1}','${viewInfo.type}','${viewInfo.anum}')">이전</a>
				</c:if>
				<span class="num">
				<c:forEach begin="${viewInfo.startPage}" end="${viewInfo.endPage < viewInfo.lastPage ? viewInfo.endPage : viewInfo.lastPage}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<a href="#none" onclick="themeSort('${i}','${viewInfo.type}','${viewInfo.anum}')">[${i}]</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</span>
				<c:if test="${viewInfo.endPage < viewInfo.lastPage}">
					<a href="#none" class="next" onclick="themeSort('${viewInfo.endPage+1}','${viewInfo.type}','${viewInfo.anum}')">다음</a>
					<a href="#none" class="end" onclick="themeSort('${viewInfo.lastPage}','${viewInfo.type}','${viewInfo.anum}')">마지막</a>
				</c:if>
			</div>
		</div>
	</div>
